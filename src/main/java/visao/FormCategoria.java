...
public FormCategoria(Categoria c) {
    initComponents();
    setDefaultCloseOperation(FormCategoria.DISPOSE_ON_CLOSE);
    this.c = c;
    this.isEditMode = true;

    // Preencher campos com os dados da categoria recebida
    jInputTextNomeCateg.setText(c.getNome());
    jComboBoxTamanho.setSelectedItem(c.getTamanho());
    jComboBoxEmbalagem.setSelectedItem(c.getEmbalagem());
    
    // Alterar o texto do botão para "Salvar"
    botaoCadastrarCategoria.setText("Salvar");
}
    ...
    if (!nome.isEmpty()) {
        ...
        if (isEditMode && c != null) {
            categoria.setId(c.getId()); // manter o ID original para atualizar
            sucesso = CategoriaDAO.updateCategoria(categoria);
            ...
            sucesso = CategoriaDAO.inserirCategoria(categoria);
            if (sucesso) {
